...
Keš je použita v instanci v případě přístupu k relaci atributu cizího klíče. Toto však platí pouze, pokud relace odkazuje na konkrétní objekt. Keš se u relací M:N nebo zpětných relací u instance nepoužije \cite{dj-qscache}. \\
...
\paragraph{Ukládání změn} Od verze 1.5 Django ORM umožňuje při ukládání instance uložit pouze specifikované atributy. Je tak možno uložit pouze změněná data a nezatěžovat databázi odesíláním všech atributů, což se v případě obsáhlého modelu může negativně projevit na výkonu. ORM však neposkytuje žádný mechanismus, který by tento přístup automatizoval, je tak potřeba implementovat vlastní sledování změn \cite{dj-save}.
...
Django ORM je velmi... avšak má i své nevýhody.
...
nefunguje limit a filter pro prefetch\_related
...
QuerySet objekt reprezentuje kolekci objektu z databaze, all, filter, exclude pouzivaji field lookups - 
...
COMMIT;
dave@edge:~/workspace/dj-ng-shop/server$ python manage.py sqlall myapp
BEGIN;
CREATE TABLE "myapp_blog" (
    "id" serial NOT NULL PRIMARY KEY,
    "name" varchar(100) NOT NULL,
