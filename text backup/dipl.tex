...
\begin{table}
  ...
  \begin{tabular}{|c|c|c|}
    ...
    \hline
    Lokální server (PostgreSQL 8.4) & 12.22 & 5.26 \\
    \hline
    Vzdálený server (PostgreSQL 8.4) & - & - \\
    \hline
  \end{tabular}
...
při používání je potřeba načist struktur nebo se ridit podle pravidel.
se nestará o ukládání a tím pádem ani validaci dat.

\subsection{Dotazování}

Při tvorbě syntaxe knihovny pypg byl kladen zřetel na jednoduchost a čitelnost dotazů, které se provádějí na pozadí. Z části byla syntaxe inspirována dotazovacím API web2py DBAL a SQLAlchemy a knihovnou NotORM. Podle mého názoru je tedy k jejímu porozumění potřeba minimum znalostí, pokud je uživatel již obeznámený se syntaxí SQL.

\begin{lstlisting}[style=custompython, label=src:pypg, caption={Ukázka získávání dat z databáze pomocí knihovny pypg}]
