...
Podobně jako u DAL se i pod zkratkou ORM (Object-relational mapper) někdy označují rámce implementující O/R mapovaní. Tyto rámce kromě O/R mapování obsahují funkce potřebné pro komunikaci s relační databází popsané výše v sekci \ref{subsec:Data-layer}. Někdy i využívají existujících DAL, které tyto funkce obsahují a přidávají k nim O/R mapování. Většinou obsahují také DBAL. \\
...
\InsertFigure{inheritance2.png}{150mm}{Ukázka návrhových vzorů pro mapování dědičnosti \cite[str. 78-79]{fowler}}{fig:Inheritance}
...
\section{Motivy vytvoření vlastní knihovny}
...
Objektově-relační mapování je v Djangu inspirováno návrhovým vzorem Active Record. Django pojmenovává každou ze tříd, které abstrahují databázovou tabulku, jako model. Model musí dědit z třídy \lstinline[style=custompython]|django.db.models.Model| a jeho atributy jsou definovány pomocí instancí potomků třídy \lstinline[style=custompython]|django.db.models.Field|. Výpis \ref{src:Mapping} zobrazuje příklad modelu Person. Django pomocí tohoto modelu vygeneruje jeho SQL reprezentaci, kterou zobrazuje výpis \ref{src:SQLMapping}. \\

\begin{lstlisting}[style=custompython, emph={Person}, emphstyle={\bfseries}, label=src:Mapping, caption=Model Person]
...
\begin{lstlisting}[label=src:SQLMapping, caption=Reprezentace modelu Person v SQL, style=customsql]
...
Django lze používat i s existující databází. Z existující databáze lze vygenerovat pro každou tabulku automaticky příslušné modely. Django podporuje i více databází. Mezi podporované databáze patří SQLite, PostgreSQL, MySQL a Oracle. Databáze lze měnit, ale znamená to menší podporu konkrétních funkcí.

\subsubsection{Dotazování}

\subsubsection{Výkon Django ORM}
...
indexy, lze je vytvorit z djanga
